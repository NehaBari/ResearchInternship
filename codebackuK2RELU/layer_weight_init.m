function [B, J] = layer_weight_init(Q, R, T, K, M, N)
...
%weight vectors such that they satisfy the given order parameters.
    ...
    for n=1:M
       B_r(:,n) = normalize(randn(N,1), 'norm'); 
    end

    for i=1:K
       J_r(:,i) = normalize(randn(N,1), 'norm'); 
    end
    ...
    for n=2:M
        ...
        %add random vector
        Bn = Bn + c(n) * B_r(:,n);
        %Linear symbolic equation created
...
%         for i=1:n-1
%             %dot B(:,i) met B(:,n) en stel gelijk aan T(i,n)
%             eq = dot(B(:,i),Bn) - T(i,n);
%             eqns(end+1) = eq;
%         end
