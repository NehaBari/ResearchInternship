clear all;
clc;
%alphaArray = 10*linspace(2,12,21);
alphaArray = linspace(10,29,20);


iterations = 10;
generalizationErrorpleatuematrix = zeros(iterations,length(alphaArray(1,:)));
generalizationErrorconvergedmatrix = zeros(iterations,length(alphaArray(1,:)));
for iteration = 1:iterations
      iteration
    
    for alphas = 1:length(alphaArray(1,:))
         ...
         [student_weights, teacher_weights] = studentTeacherInit(K,N,strInitType);
          if (strcmp(strInitType,'with overlap'))
            ...
            for n = 1:trainSteps
             ...
             generalizationErrorpleatuematrix(iteration,alphas) = gen_error(midrowpleatue);
             generalizationErrorconvergedmatrix(iteration,alphas) = gen_error(midrowconverged);
    end
